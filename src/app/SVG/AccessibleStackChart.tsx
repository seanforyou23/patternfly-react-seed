...
const AccessibleStackChart: React.FunctionComponent<{}> = () => (
  <PageSection>
    <TextContent>
      ...
      <Text>This strategy is expected to break down when it comes to time series charts as individual datapoints become much less useful and what's more interesting are insights like trends over time, averages, and other types of measurements that are combinatorial in nature and can't be relayed by consuming any single element. I flirted with this idea by adding an <code>aria-describedby="..."</code> to each tspan element along the x-axis which describes an example of one of these, we'll call them "summaryAxioms". The idea is to add to each PF chart a new prop which represents a list of the summarization statements which can be applicable to the given chart. These can be pre-canned by chart type, for example butllet charts deal with multiple measurements against a thresholds and donut charts deal with portions against a whole. For this simple stacked bar chart example, you can see I simply calculated the total value of all items in each x-axis and crafted that into a statement that gives the high-level insight back to the user. It's totally subjective as to what insights/take-aways might be most important for a chart, and this likely depends on more of the exact use case than just the chart type, <em>however</em>, this isn't a reason not to make some small progress in the right direction. While without proper research we may not be able to say which summary-type metrics may be most appropriate to screen reader users, I think we can make educated guesses based on the existing <a href="https://www.patternfly.org/v4/design-guidelines/usage-and-behavior/charts#chart-types" rel="nofollow noopener" target="_blank">usage and behavior for PF supported charts</a>. Anyway, here's the chart!</Text>
    </TextContent>
