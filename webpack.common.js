...
const API_ENDPOINT_KEY = 'API_BASE';
const logColors = {
  blue: '\x1b[36m',
  reset: '\x1b[0m'
}
...
module.exports = env => {
  let envObj = { "development": '', "production": '' }[env];

  if (fs.existsSync(path.resolve(__dirname, '.env'))) {
    envObj = require('dotenv').config();
    Object.keys(envObj && envObj.parsed).forEach(envKey => {
      envObj = JSON.parse(envObj.parsed[envKey])[env];
    });

    console.log(
      `${logColors.blue}%s${logColors.reset}`,
      `URI configured for process.env.${API_ENDPOINT_KEY}: ${envObj}\n`
    );
  }
  ...
  return {
    ...
    plugins: (env === 'production') ? [
      ...
      new webpack.DefinePlugin({
        'process.env': {
          [`${API_ENDPOINT_KEY}`]: JSON.stringify(envObj),
        }
      }),
